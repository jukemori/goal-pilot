  ...
  if (!completion) {
    ...
    try {
      ...
      if (!roadmapData.phases || !Array.isArray(roadmapData.phases) || roadmapData.phases.length < 3) {
        console.error('Incomplete response: only', roadmapData.phases?.length || 0, 'phases generated')
        throw new Error(`Incomplete AI response: only ${roadmapData.phases?.length || 0} phases generated, expected 6-12`)
      }
      
      console.log('Complete response validated:', roadmapData.phases.length, 'phases generated')
...
export async function generateRoadmapOverview(goalId: string) {
  ...
  const { data: roadmap, error: roadmapError } = await supabase
    ...
    .insert({
      goal_id: goalId,
      ai_generated_plan: { overview_generated: true, ...roadmapOverview } as unknown as Json,
...
export async function generateRoadmapStages(roadmapId: string) {
  ...
  try {
    ...
    if (!stagesData.phases || !Array.isArray(stagesData.phases) || stagesData.phases.length < 3) {
      console.error('Incomplete stages response: only', stagesData.phases?.length || 0, 'phases generated')
      throw new Error(`Incomplete AI response: only ${stagesData.phases?.length || 0} phases generated, expected 6-12`)
    }
    
    console.log('Complete stages response validated:', stagesData.phases.length, 'phases generated')
