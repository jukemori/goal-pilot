class PerformanceAnalysisPlugin {
  ...
  apply(compiler) {
    ...
    compiler.hooks.done.tap("PerformanceAnalysisPlugin", (stats) => {
      ...
      // Log large assets
      const largeAssets = assetSizes.filter(
        (asset) => asset.size > this.options.maxAssetSize,
      );
      if (largeAssets.length > 0) {
        ...
        largeAssets.forEach((asset) => {
          ...
          const emoji =
            asset.size > this.options.maxAssetSize * 2 ? "🔴" : "🟡";
      ...
      if (modules.length > 0) {
        ...
        modules.slice(0, 10).forEach((module) => {
          ...
          console.log(
            `  📚 ${module.name}: ${sizeKB}KB (${module.reasons} reasons)`,
          );
        });
      }

      // Bundle composition analysis
      const totalSize = assetSizes.reduce(
        (total, asset) => total + asset.size,
        0,
      );
      const jsSize = assetSizes
        .filter((a) => a.type === "js")
        .reduce((total, asset) => total + asset.size, 0);
      const cssSize = assetSizes
        .filter((a) => a.type === "css")
        .reduce((total, asset) => total + asset.size, 0);
      ...
      console.log(
        `  📊 JavaScript: ${(jsSize / 1024).toFixed(2)}KB (${((jsSize / totalSize) * 100).toFixed(1)}%)`,
      );
      console.log(
        `  📊 CSS: ${(cssSize / 1024).toFixed(2)}KB (${((cssSize / totalSize) * 100).toFixed(1)}%)`,
      );
      ...
      if (largeAssets.length > 0) {
        console.log(
          `  • Consider code splitting for ${largeAssets.length} large asset(s)`,
        );
      }
